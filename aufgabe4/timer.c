...
ssize_t timer_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos){
  int transferred = 0;  
  ...
  switch(dev->state){
	...
	if(jif < dev->jiffies_start){
	  ...
	  transferred = MIN(46, count);
	  if(copy_to_user(buf, "Sorry, there was a wrap around - resetting...\n", transferred)){
	    printk(KERN_WARNING "Cannot send data to user!\n");
	    return -EFAULT;
	  }
	  dev->state = READY;
	  ...
	  sprintf(my_buf, "%llu\n", ((jif - dev->jiffies_start) - dev->jiffies_pause_total));
	...
	transferred = MIN(21, count);
	if(copy_to_user(buf, "Unsupported for now.\n", transferred)){
	  printk(KERN_WARNING "Cannot send data to user!\n");
	  return -EFAULT;
	}
      }
...
ssize_t timer_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos){
  struct timer_device *dev = filp->private_data;
  char my_buf[BUFFER_SIZE] = { 0 };
  ...
  if(copy_from_user(my_buf, buf, (transferred - 1) )){
  ...
  // start command
    ...
    // if it's the forward counter, then we need to store current jiffies
    if(dev->direction == FORWARD){
  ...
  // load command
  else if(my_buf[0] == 'l' && dev->state == READY && dev->direction == BACKWARD){
...
static int timer_init( void ){
  ...
  result = cdev_add ( &(backward_timer.cdev), MKDEV(timer_major, MINOR_START + 1), 1);
  if(result < 0){
